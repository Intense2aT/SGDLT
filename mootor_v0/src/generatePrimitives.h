...
// makes a triangle with the 90 degree corner at the point of generation (xpos, ypos)
...
bufferSizeStore genText(float*& vertexArray, unsigned int*& indexArray, float& letterWidth, float& letterHeight, float* padding, int* translatedTextArray, unsigned int& arraySize, float xpos, float ypos, int& setObjectAmount, int*& objectAmount);